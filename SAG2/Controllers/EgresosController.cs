...
{ 
    public class EgresosController : Controller
    {
        ...
        public ViewResult Index()
        {
            ...
            var movimiento = db.Movimiento.Include(m => m.Proyecto).Include(m => m.TipoComprobante).Include(m => m.Cuenta).Include(m => m.Persona).Include(m => m.Proveedor).Include(m => m.CuentaCorriente).Where(m => m.TipoComprobanteID == ctes.tipoEgreso).Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.Temporal == null && a.Eliminado == null && ((a.CuentaID != 6 || a.CuentaID == null) || a.CuentaID == null)).OrderByDescending(a => a.Periodo).ThenByDescending(a => a.NumeroComprobante);
        ...
        public ActionResult Create()
        {
            ...
            else
            {
                ...
                {
                    ViewBag.NroComprobante = db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID == ctes.tipoEgreso).Where(a => a.Periodo == periodo).Max(a => a.NumeroComprobante) + 1;
                }
            }
        ...
        public ActionResult Create(Movimiento egreso)
        {
            ...
            if (senainfo)
            {
                if (db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID != ctes.tipoIngreso).Where(a => a.Periodo == periodo).Count() > 0)
                {
                    egreso.NumeroComprobante = db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID != ctes.tipoIngreso).Where(a => a.Periodo == periodo).Max(a => a.NumeroComprobante) + 1;
                }
            }
            else
            {
                if (db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID == ctes.tipoEgreso).Where(a => a.Periodo == periodo).Count() > 0)
                {
                    egreso.NumeroComprobante = db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID == ctes.tipoEgreso).Where(a => a.Periodo == periodo).Max(a => a.NumeroComprobante) + 1;
                }
            }
            ...
            try
            {
                ...
                if (Session["DetalleEgreso"] != null)
                {
                    ...
                    foreach (DetalleEgreso detalle in lista)
                    {
                        ...
                        detalleEgreso = detalle;
                        db.DetalleEgreso.Add(detalleEgreso);*/
        ...
        public ActionResult Edit(int id, string imprimir = "")
        {
            ...
            try
            {
                ViewBag.MaxComprobante = db.Movimiento.Where(m => m.ProyectoID == egreso.ProyectoID && m.TipoComprobanteID == ctes.tipoEgreso && m.Periodo == periodo).Max(a => a.NumeroComprobante).ToString();
            }
            catch (Exception)
            { }
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (Session["DetalleEgreso"] != null)
                    {
                        ...
                        // Borramos detalle original e ingresamos el nuevo listado
                        //List<DetalleEgreso> original = db.DetalleEgreso.Where(d => d.MovimientoID == egresoID).ToList();
                        //foreach (DetalleEgreso tmp in original)
                        //{
                            //db.DetalleEgreso.Remove(tmp);
                            //db.SaveChanges();
                        //}
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            Proyecto Proyecto = (Proyecto)Session["Proyecto"];
            int maxComprobante = db.Movimiento.Where(m => m.ProyectoID == Proyecto.ID).Where(a => a.TipoComprobanteID == ctes.tipoEgreso).Where(a => a.Periodo == periodo).Max(a => a.NumeroComprobante);
        ...
        public ActionResult Anular(int id)
        {
            ...
            {
                movimiento.Nulo = "S";
                movimiento.Monto_Egresos = monto;
                db.Entry(movimiento).State = EntityState.Modified;
                db.SaveChanges();
            }
