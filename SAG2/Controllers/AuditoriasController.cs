...
namespace SAG2.Controllers
{
    public class AuditoriasController : Controller
    {
        ...
        public ActionResult Informe()
        {
            ...
            try
            {
                ViewBag.LineasAtencionID = new SelectList(db.LineasAtencion, "ID", "NombreLista", InformeAuditoria.LineasAtencionID);
            }
            catch (Exception)
            {
        ...
        public ActionResult Informe(InformeAuditoria InformeAuditoria)
        {
            ...
            if (Request.QueryString["Documentos"] == null || !Request.QueryString["Documentos"].ToString().Equals("NO"))
            {
                ...
                for (int i = 1; i < 20; i++)
                {
                    try
                    {
                        if (Request.Form["Documento_" + i] != null && !Request.Form["Documento_" + i].ToString().Equals(""))
                        {
                            //Response.Write(Request.Form["Documento_" + i].ToString());
                            documentos.Add(Request.Form["Documento_" + i].ToString());
                        }
                    }
                    catch (Exception e)
                    {
                ...
                InformeAuditoria.Documentos = string.Join(",", documentos.ToArray());
            }
        ...
        public ActionResult ReporteIndividual(int Informe = 0)
        {
            ...
            ViewBag.CumplimientoPersonal = (ok / total)*100;
